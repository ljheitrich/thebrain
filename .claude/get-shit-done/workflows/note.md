...
- **Global scope**: `C:/The Brain/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/The Brain/.claude/notes/`)
...
2. Glob `C:/The Brain/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/The Brain/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/The Brain/.claude/notes/` — works in any directory
